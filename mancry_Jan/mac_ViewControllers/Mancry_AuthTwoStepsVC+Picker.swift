...
extension Mancry_AuthTwoStepsVC {
    ...
    func mancry_showBottomPicker(for item: Mancry_AuthTwoKYCItem, at index: Int) {
        mancry_bottomPicker?.dismiss()
        mancry_bottomPicker = nil
        
        let pack = Mancry_AuthTwoOptionLoom.mancry_packFromButtonList(item.buttonList, currentValue: item.value)
        
        let picker = Mancry_customselectPopupView()
        mancry_bottomPicker = picker
        
        picker.configure(title: item.itemName, data: pack.labels, defaultIndex: pack.defaultIndex)
        
        picker.onConfirm = { [weak self] selectedIndex, selectedLabel in
            guard let self = self else { return }
            
            if let key = Mancry_AuthTwoOptionLoom.mancry_key(at: selectedIndex, in: pack) {
